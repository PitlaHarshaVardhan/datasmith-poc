...
def build_vector_store_if_needed():
    if _collection.count() > 0:
        logger.info("Chroma collection already initialized with %d docs", _collection.count())
        return
    logger.info("Building vector store from nephrology_reference.txt")
    text = REF_FILE.read_text(encoding="utf-8")
    chunks = _chunk_text(text)
    ids = [f"chunk-{i}" for i in range(len(chunks))]
    metadatas = [{"source": "nephrology_reference.txt", "chunk_id": i} for i in range(len(chunks))]
    ...
    for doc, meta, dist in zip(
        ...
        docs.append(
            RetrievedDoc(
                ...
                score=float(1 - dist)  # cosine sim approx
            )
        )
